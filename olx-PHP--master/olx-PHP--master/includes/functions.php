...
function getOwnerPhoneNumber($AdsID) {
    // Assuming you are using MySQLi
    $clink = mysqli_connect("localhost", "root", "rahul", "olx");
    ...
    $AdsID = mysqli_real_escape_string($clink, $AdsID);

    $query = "SELECT u.Phone 
              FROM users u
              INNER JOIN advertisments a ON u.UserID = a.UserID
              WHERE a.AdsID = $AdsID";
    ...
    if ($result) {
        ...
        echo "Error retrieving owner's phone number: " . mysqli_error($clink);
        return false;
    }
